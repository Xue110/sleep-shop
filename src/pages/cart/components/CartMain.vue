...
import { checkAllCartAPI, deleteMemberCartAPI, getCartList, putMemberCartBySkuIdAPI } from '@/services/cart';
...
const onChangeCount = async (ev:InputNumberBoxEvent) => {
  const res = await putMemberCartBySkuIdAPI(ev.index,{ count:ev.value })
  getMemberCartData()
}
// 修改单品选中状态
...
const onChangeSelectedAll = () => {
  ...
  //后端数据更新
  checkAllCartAPI({ selected: _isSelectedAll })
}
// 计算选中单品列表
...
const selectedCartTotal = computed(() => {
  return selectedCartList.value.reduce((total, item) => total + item.nowPrice * item.count, 0)
})
// 计算选中总件数
  ...
  <scroll-view scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      ...
      <view class="cart-list" v-if="cartList.length">
        ...
        <uni-swipe-action>
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <!-- 右侧删除按钮 -->
            <template #right>
              <view @tap="deleteCartItem(item.skuId)" class="cart-swipe-right">
      ...
      <view class="toolbar">
        ...
        <text class="amount">{{selectedCartTotal}}</text>
        <view  class="button-grounp">
          <view @tap="gotoPayment" class="button payment-button" :class="{ disabled: !selectedCartCount }"> 去结算({{ selectedCartCount }}) </view>
        </view>
      </view>
    </template>
    <!-- 未登录: 提示登录 -->
